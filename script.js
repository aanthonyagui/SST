...
function initParticles() {
    ...
    for(let i=0; i<70; i++) {
        particles.push({
            ...
            vx: (Math.random() - 0.5) * 0.4, vy: (Math.random() - 0.5) * 0.4, size: 1.5
        });
    }
}
function animate() {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.fillStyle = '#00d2ff'; ctx.strokeStyle = 'rgba(0, 210, 255, 0.1)';
...
async function mostrarMenu(user) {
    ...
    if (menus) {
        menus.forEach(m => {
            ...
            card.onclick = () => alert("Abriendo " + m.titulo);
...
document.getElementById('admin-add-menu').onclick = () => {
    ...
    iconosSST.forEach(icon => {
        ...
        div.onclick = () => crearNuevoMenu(icon);
        lista.appendChild(div);
    });
};

async function crearNuevoMenu(icon) {
    const titulo = prompt("Nombre de la nueva función:");
    if (!titulo) return;
    const soloAdmin = confirm("¿Es solo para administradores?");
    const { error } = await _supabase.from('config_menus').insert([{ titulo, icono: icon, solo_admin: soloAdmin, color: '#00d2ff' }]);
