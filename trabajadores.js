// trabajadores.js - VERSIÓN FINAL: PDF ROBUSTO (NO FALLA) + ATS LINK DIRECTO
...
export async function cargarModuloTrabajadores(contenedor, supabase, empresa) {
    ...
    window.imprimirDoc = async (tipo) => {
        ...
        if (tipo === 'ficha') { try { await generarPDF_Ficha(id); } catch (e) { alert("Error PDF: " + e.message); } }
        else if (tipo === 'ats') window.open('ATS.pdf', '_blank'); // ABRE ARCHIVO ESTÁTICO
